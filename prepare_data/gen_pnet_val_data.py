...
config.neg_inside_num_per_image = 5
...
# WIDER-FACE val image and val annotation path
data_root_dir = '/home/dafu/data/WIDER_FACE'
anno_file = os.path.join('./prepare_data/wider_val_annotation.txt')

image_root_dir = os.path.join(data_root_dir, 'WIDER_val/images')
# Save patch image path
save_dir_root = '/home/dafu/data/jdap_data'
neg_save_dir = os.path.join(save_dir_root, "%d/val_negative" % net_size)
pos_save_dir = os.path.join(save_dir_root, "%d/val_positive" % net_size)
save_dir = os.path.join(save_dir_root, "%d" % net_size)
for dir_path in [save_dir, neg_save_dir, pos_save_dir]:
    if not os.path.exists(dir_path):
        os.makedirs(dir_path)
# Create output file
f1 = open(os.path.join(save_dir, 'val_pos_%d.txt' % net_size), 'w')
f2 = open(os.path.join(save_dir, 'val_neg_%d.txt' % net_size), 'w')
...
for annotation in annotations:
    ...
    while neg_num < config.neg_outside_num_per_image:
        ...
        if np.max(Iou) < config.neg_iou:
            ...
            f2.write("12/val_negative/%s.jpg" % n_idx + ' 0 0 0 0 0\n')
    ...
    for box in boxes:
        ...
        if best_x2 <= width and best_y2 <= height:
            ...
            f1.write("12/val_positive/%s.jpg" % p_idx + ' 1 %.2f %.2f %.2f %.2f\n'
